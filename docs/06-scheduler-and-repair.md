...
**Regional partition (chaos harness):** `harness partition --region <r>` freezes those agent containers with `docker compose pause` (cgroup freezer). A bridge-level `docker network disconnect` does not restore published-port DNAT on reconnect, so it cannot heal without a restart — and a restart is a flap, not a partition heal. The region cap of 3 means cutting one region leaves every chunk at ≥ 13 healthy. If the freeze lasts past `HEALTH_OFFLINE_AFTER`, repair re-places the lost fragments; unpause resumes the same process and IP, and flap handling on `node.online` restores copies that were not reconstructed elsewhere and expires surplus copies so the region cap cannot grow past 3.
