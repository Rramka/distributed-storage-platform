...
For the MVP, verification uses **pre-computed challenge sets minted by the Health Monitor** (the spec refresh path). The Health Monitor fetches a fragment once over a metadata-issued retrieval ticket, verifies its SHA-256, computes N future `(offset, length, nonce, expected)` tuples, stores the tuples (never the bytes), and spends them one at a time. The client does **not** register challenge responses at upload commit. When a set runs low, the same GET-and-compute refresh repeats. Repair workers seed a set the same way after a flap GET. `GET /challenge` is authorized by a metadata-minted `challenge` ticket — Metadata remains the sole holder of `TICKET_SIGNING_SEED`.
