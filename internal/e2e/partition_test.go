...
// TestRegionalPartitionKeepsDownloads closes 3 of 16 agents (one region under
// the region cap) and asserts the remaining 13 shards still reconstruct.
func TestRegionalPartitionKeepsDownloads(t *testing.T) {
	t.Parallel()
	platformCA, err := ca.EnsureCA(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	seed := bytes.Repeat([]byte{0x45}, 32)
	...
	for i := 0; i < regionCut; i++ {
		nodes[i].srv.Close()
	}

	partial := make([][]byte, pipeline.ECTotal)
	healthy := 0
	for i := regionCut; i < nAgents; i++ {
		tk, err := signer.Sign(tickets.Ticket{
	...
	if healthy < floor {
		t.Fatalf("healthy %d below floor %d", healthy, floor)
	}
