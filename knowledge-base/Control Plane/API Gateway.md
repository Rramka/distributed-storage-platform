...
Does **not** sit on the fragment byte path. After [[Upload Flow]] planning, the client talks to [[Node Agent]] endpoints directly. `GET /storage` and `GET /earnings` proxy to the [[Ledger Service]].
